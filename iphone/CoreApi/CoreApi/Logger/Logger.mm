...
+ (nullable NSURL *)getLogFileURL {
  if ([self fileLoggingEnabled]) {
    ...
    if (@available(iOS 15.0, *)) {
      ...
      OSLogEnumerator * enumerator = [store entriesEnumeratorWithOptions:OSLogEnumeratorReverse position:nil predicate:predicate error:&error];
      ...
      if (logString.length == 0) {
        LOG(LDEBUG, ("OSLog entry is empty."));
        return nil;
      }
